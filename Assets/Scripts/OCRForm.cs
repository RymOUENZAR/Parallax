...
public class OCRForm
{
    ...
    private static String OcrImage(Tesseract ocr, Mat image, OCRMode mode, Mat imageColor)
    {
        ...
        else
        {
            ...
            Rectangle[] regions;
            using (
                ERFilterNM1 er1 = new ERFilterNM1("Assets/ClassifiersXML/trained_classifierNM1.xml", 8, 0.00025f, 0.13f, 0.4f, true, 0.1f))
            using (ERFilterNM2 er2 = new ERFilterNM2("Assets/ClassifiersXML/trained_classifierNM2.xml", 0.3f))
            {
    ...
    public void OcrImage(Mat source)
    {


        Mat result = new Mat();
        String ocredText = OcrImage(_ocr, source, OCRMode.TextDetection, result);

        CvInvoke.PutText(result, ocredText, new Point(0, 0), FontFace.HersheyComplex, 2, new MCvScalar(255, 0, 0));
    }
}
